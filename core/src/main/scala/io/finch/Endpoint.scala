...
trait Endpoint[A] { self =>
  ...
  final def mapAsync[B](fn: A => Future[B]): Endpoint[B] = new Endpoint[B] {
    final def apply(input: Input): Endpoint.Result[B] = self(input) match {
      case m: EndpointResult.Matched[A] =>
        m.o = m.out.flatMapF(oa => oa.traverse(a => fn(a)))
        m.asInstanceOf[EndpointResult[B]]
      case _ => EndpointResult.empty[B]
    }
  ...
  final def transform[B](fn: Future[Output[A]] => Future[Output[B]]): Endpoint[B] =
    new Endpoint[B] {
      final def apply(input: Input): Endpoint.Result[B] = self(input) match {
        case m: EndpointResult.Matched[A] =>
          val out = m.out
          m.o = Rerunnable.fromFuture(fn(out.run))
          m.asInstanceOf[EndpointResult[B]]
        case _ => EndpointResult.empty[B]
      }
  ...
  final def product[B](other: Endpoint[B]): Endpoint[(A, B)] = new Endpoint[(A, B)] {
    ...
    final def apply(input: Input): Endpoint.Result[(A, B)] = self(input) match {
      case a: EndpointResult.Matched[A] => other(a.rem) match {
        case b: EndpointResult.Matched[B] =>
          b.o = a.out.liftToTry.product(b.out.liftToTry).flatMapF(join)
          b.asInstanceOf[EndpointResult[(A, B)]]
        case _ => EndpointResult.empty[(A, B)]
  ...
  final def |[B >: A](other: Endpoint[B]): Endpoint[B] = new Endpoint[B] {
    final def apply(input: Input): Endpoint.Result[B] = self(input) match {
      case a: EndpointResult.Matched[A] => other(input) match {
        case b: EndpointResult.Matched[B] =>
          if (a.rem.path.length <= b.rem.path.length) a.asInstanceOf[EndpointResult[B]] else b
        case _ => a.asInstanceOf[EndpointResult[B]]
      }
      case _ => other(input)
    }
  ...
  final def lift: Endpoint[Option[A]] = new Endpoint[Option[A]] {
    final def apply(input: Input): Endpoint.Result[Option[A]] = self(input) match {
      case m: EndpointResult.Matched[A] =>
        m.o = m.out.liftToTry.map(toa =>
          toa.toOption.fold(Output.None: Output[Option[A]])(o => o.map(Some.apply))
        )
        m.asInstanceOf[EndpointResult[Option[A]]]
      case _ => EndpointResult.empty[Option[A]]
    }
...
object Endpoint {
  ...
  def empty[A]: Endpoint[A] = new Endpoint[A] {
    final def apply(input: Input): Result[A] = EndpointResult.empty[A]
  }
  ...
  def const[A](a: A): Endpoint[A] = new Endpoint[A] {
    final def apply(input: Input): Result[A] = EndpointResult(input, Rs.const(Output.payload(a)))
  }
  ...
  def lift[A](a: => A): Endpoint[A] = new Endpoint[A] {
    final def apply(input: Input): Result[A] = EndpointResult(input, Rerunnable(Output.payload(a)))
  }
  ...
  def liftFuture[A](fa: => Future[A]): Endpoint[A] = new Endpoint[A] {
    final def apply(input: Input): Result[A] =
      EndpointResult(input, Rerunnable.fromFuture(fa).map(a => Output.payload(a)))
  }
  ...
  def liftOutput[A](oa: => Output[A]): Endpoint[A] = new Endpoint[A] {
    final def apply(input: Input): Result[A] = EndpointResult(input, Rerunnable(oa))
  }
  ...
  def liftFutureOutput[A](foa: => Future[Output[A]]): Endpoint[A] = new Endpoint[A] {
    final def apply(input: Input): Result[A] = EndpointResult(input, Rerunnable.fromFuture(foa))
  }
